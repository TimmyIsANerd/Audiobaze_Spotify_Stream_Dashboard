module.exports = {
  friendlyName: "Send template gmail",

  description: "Send an email via Gmail using Template.",
  ...
  inputs: {
    ...
    ensureAck: {
      description:
        "Whether to wait for acknowledgement (to hear back) that the email was successfully sent (or at least queued for sending) before retuirnng.",
      extendedDescription:
  ...
  fn: async function ({
    ...
    sails.log.info("..... Node Mailer Service Started.....");
    var nodemailer = require("nodemailer");

    const gmailAccount = {
      email: "audiobazze@gmail.com",
      password: "Audio006$",
    };
    ...
    const EmailService = async () => {
      ...
      let transporter = nodemailer.createTransport({
        ...
        auth: {
          user: gmailAccount.email, // Gmail Account
          pass: gmailAccount.password, // Gmail Password
        },
      });
      ...
      transporter.verify(async function (error, success) {
        if (error) {
          ...
          sails.log.info("Server is ready to send email");
          ...
          if (dontActuallySend) {
            ...
            var sendMail = await transporter.sendMail(messageData);
          }
        }
      });
    };

    EmailService().catch((err) => sails.log.info(err));
  },
};
