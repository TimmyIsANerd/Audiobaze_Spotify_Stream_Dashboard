...
and exposed as \`req.me\`.)`,
  ...
  fn: async function ({ emailAddress, password, rememberMe, machineId }) {
    ...
    // Check if Activated License Exists
    const userLicense = await User.findOne({
      emailAddress: emailAddress.toLowerCase(),
    });

    // Check if License Data exists
    if (userLicense.licenseData === null) {
      return res.json({
        status: "No License Activated",
      });
    }

    // Check if License exits
    if (userLicense.licenseData) {
      var today = new Date();
      var today = today.toLocaleDateString();
      const data = JSON.parse(userLicense.licenseData);

      const licenseExpiryDate = data.expiryDate;
      ...
      if (licenseExpiryDate === today) {
        ...
        return res
          .json({
            status: "Key Expired",
            message:
              "Your Audiobaze License Key is expired, please purchase a new license to use Audiobaze services.",
          })
          .status(200);
      }
    }

    // Add New User Machine ID
    if (userLicense.licenseData) {
      var data = JSON.parse(userLicense.licenseData);
      const machineIDs = data.machineId;

      // Check for duplicate Machine IDs and remove them from slot
      const ids = machineIDs;
      const cleanMachineIdslot = Array.from(new Set(ids));

      if (cleanMachineIdslot.length < 4) {
        cleanMachineIdslot.pop(machineId);
        data.machineId = cleanMachineIdslot;
        ...
        return res
          .json({
            status: "Max No. of Devices Logged In",
            message:
              "You can only login to a maximum of 4 devices on our systems",
          })
          .status(500);
      }
    }
